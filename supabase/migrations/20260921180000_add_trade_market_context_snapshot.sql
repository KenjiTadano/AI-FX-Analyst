-- Task104: AI-independent market context snapshot as nullable JSONB.
-- Legacy rows stay NULL. RLS unchanged. Immutable after insert like analysis_snapshot / exit_plan.
begin;

alter table public.trades
  add column if not exists market_context_snapshot jsonb
  check (market_context_snapshot is null or jsonb_typeof(market_context_snapshot) = 'object');

comment on column public.trades.market_context_snapshot is
  'Immutable market context at trade registration (MTF/Regime/technical). AI-independent. version=1 JSON. NULL for legacy.';
...
begin
  ...
  insert into public.trades(user_id, local_trade_id, pair, side, status, quantity, entry_price, exit_price, opened_at, closed_at, stop_loss, take_profit, realized_pnl, notes, analysis_snapshot, exit_plan, market_context_snapshot, created_at, updated_at)
  select auth.uid(), r.local_trade_id, r.pair, r.side, r.status, r.quantity, r.entry_price, r.exit_price, r.opened_at, r.closed_at, r.stop_loss, r.take_profit, r.realized_pnl, r.notes, r.analysis_snapshot, r.exit_plan, r.market_context_snapshot, r.created_at, r.updated_at
  from jsonb_to_recordset(payload) as r(
    ...
    analysis_snapshot jsonb, exit_plan jsonb, market_context_snapshot jsonb, created_at timestamptz, updated_at timestamptz
  )
