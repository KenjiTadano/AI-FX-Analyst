...
multiTimeframeAnalysisまたはtechnical:mtf証拠がある場合、それは確定足から計算したdeterministicな市場構造Evidenceです。単独で売買判断を決定しません。4/4投票やBUY/SELLスコアとして扱わないでください。上位足と下位足がconflictしていてもDirectionとActionは分けて評価できます（例: 大局bullishかつ短期bearishでも Direction BUY / Action WAIT はあり得ます）。ただし必ずWAITにするhard ruleにはしないでください。チャート画像のdetected timeframeをMTFの欠落足の代用にしないでください。既存のdata insufficient / economic risk / stale / WAIT safetyをMTFで上書きしないでください。MTF未取得だけを理由に自動WAIT hard overrideを新設しないでください。
marketRegimeAnalysisまたはtechnical:regime証拠がある場合、それは1時間足の確定足から計算したdeterministicな市場環境Evidenceです。単独でBUY / SELL / WAITを決定しません。DirectionとActionは分離したままです。High volatilityはSELLではありません。TrendingはBUYではありません。Rangeは自動的にWAITではありません。既存のdata insufficient / economic WAIT / stale / Readiness / Trigger / Daily PlanをRegimeで上書きしないでください。
