...
export function Dashboard({ analyses, account }: { analyses: FxAnalysis[]; account: Account }) {
  ...
  return <>
    <header className="app-header"><div className="header-inner"><Link className="brand" href="/" aria-label="AI FX Analyst ホーム"><span className="brand-icon" aria-hidden="true">↗</span><span>AI FX <span className="brand-light">Analyst</span></span></Link><span className="demo-badge"><span />MVP / TASK 010</span></div></header>
    <main className="dashboard-shell" id="main-content">
      ...
      <div hidden={view !== "chart"}><ChartAnalysisPanel pair={selectedPair} onPairChange={setSelectedPair} /></div>
      {auth.user ? <TradeJournal key={userId} userId={userId} view={view === "chart" ? "analysis" : view} pair={selectedPair} quote={quote?.data != null && quote.fetchedAt ? { pair: selectedPair, price: quote.data, fetchedAt: quote.fetchedAt, stale: !!quote.stale || !!quote.error } : null} analysis={ai?.data ?? null} initialBalance={journalBalance.userId === userId ? journalBalance.value : NaN} /> : view !== "analysis" && view !== "chart" && <LoginRequired />}
      <footer className="page-footer"><span>AI FX Analyst</span><p>分析は条件付きの参考情報です。WAITも正常な判断です。</p><span>PROTOTYPE / TASK 010</span></footer>
    </main>
  </>;
}
