...
export function Dashboard({ analyses, account }: { analyses: FxAnalysis[]; account: Account }) {
  ...
  return <>
    <header className="app-header"><div className="header-inner"><Link className="brand" href="/" aria-label="AI FX Analyst ホーム"><span className="brand-icon" aria-hidden="true">↗</span><span>AI FX <span className="brand-light">Analyst</span></span></Link><span className="demo-badge"><span />MVP / TASK 003</span></div></header>
    <main className="dashboard-shell" id="main-content">
      ...
      <div className="demo-notice"><span className="notice-icon" aria-hidden="true">i</span><p><strong>レート・テクニカルはTwelve Data</strong><span>ニュース・経済指標・AI判定・推奨トレード・資金管理はモックです。無料枠は1日800クレジットのため、常時更新には上限があります。</span></p></div>
      ...
      <div className="dashboard-grid" aria-live="polite" aria-atomic="false"><SignalPanel analysis={analysis} /><TradePanel analysis={analysis} /><CapitalPanel analysis={analysis} account={account} /><TechnicalPanel data={market?.data ?? null} error={market?.error ?? null} /><ReasonsPanel analysis={analysis} /><CommentaryPanel analysis={analysis} /></div>
      <footer className="page-footer"><span>AI FX Analyst</span><p>AI判定・推奨トレードはサンプルです。実際の投資判断には使用しないでください。</p><span>PROTOTYPE / TASK 003</span></footer>
    </main>
  </>;
}
