...
export function Dashboard({ analyses, account }: { analyses: FxAnalysis[]; account: Account }) {
  ...
  return <>
    <header className="app-header"><div className="header-inner"><Link className="brand" href="/" aria-label="AI FX Analyst ホーム"><span className="brand-icon" aria-hidden="true">↗</span><span>AI FX <span className="brand-light">Analyst</span></span></Link><span className="demo-badge"><span />MVP / TASK 005</span></div></header>
    <main className="dashboard-shell" id="main-content">
      ...
      <div className="demo-notice"><span className="notice-icon" aria-hidden="true">i</span><p><strong>レート・テクニカルはTwelve Data</strong><span>AI総合判定は取得済みデータから分析。AI未取得・データ不足時は待機します。資金は参考値です。無料枠は1日800クレジットのため、常時更新には上限があります。</span></p></div>
      ...
      <div className="dashboard-grid" aria-live="polite" aria-atomic="false"><AIOverview response={ai} pair={selectedPair} account={account} /><TechnicalPanel data={market?.data ?? null} error={market?.error ?? null} /><FundamentalPanel symbol={selectedPair} /><AIExplanation response={ai} /></div>
      <footer className="page-footer"><span>AI FX Analyst</span><p>分析は条件付きの参考情報です。WAITも正常な判断です。</p><span>PROTOTYPE / TASK 005</span></footer>
    </main>
  </>;
}
