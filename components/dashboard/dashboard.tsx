...
export function Dashboard({ analyses, account }: { analyses: FxAnalysis[]; account: Account }) {
  ...
  return <>
    <header className="app-header"><div className="header-inner"><Link className="brand" href="/" aria-label="AI FX Analyst ホーム"><span className="brand-icon" aria-hidden="true">↗</span><span>AI FX <span className="brand-light">Analyst</span></span></Link><span className="demo-badge"><span />MVP / TASK 006</span></div></header>
    <main className="dashboard-shell" id="main-content">
      ...
      <div className="dashboard-grid" aria-live="polite" aria-atomic="false"><AIOverview response={ai} pair={selectedPair} /><RiskManagement analysis={ai?.data ?? null} pair={selectedPair} currentRate={quote?.stale || quote?.error ? null : quote?.data ?? null} initialBalance={account.balance} initialTarget={account.target} /><TechnicalPanel data={market?.data ?? null} error={market?.error ?? null} /><FundamentalPanel symbol={selectedPair} /><AIExplanation response={ai} /></div>
      <footer className="page-footer"><span>AI FX Analyst</span><p>分析は条件付きの参考情報です。WAITも正常な判断です。</p><span>PROTOTYPE / TASK 006</span></footer>
    </main>
  </>;
}
