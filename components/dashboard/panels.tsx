...
export function Panel({ title, eyebrow, children, className = "", testId, ariaLabel }: { title: string; eyebrow: string; children: ReactNode; className?: string; testId?: string; ariaLabel?: string }) {
  return <section className={`panel ${className}`} data-testid={testId} aria-label={ariaLabel}><div className="section-heading"><span className="eyebrow">{eyebrow}</span><h2>{title}</h2></div>{children}</section>;
}
