...
export function Performance({ trades, initialBalance }: { trades: Trade[]; initialBalance: number }) {
  ...
  const filteredTrades = useMemo(() => filterTradesByPeriod(trades, period, now), [trades, period, now]);
  ...
  const allAvg = allStats.count ? allStats.totalPnl / allStats.count : null;
  ...
  const periodEmpty = filteredTrades.length === 0;
  const showRecentCompare = period !== "all"
    && stats.count >= MIN_INSIGHT_SAMPLE_SIZE
    && allStats.count >= MIN_INSIGHT_SAMPLE_SIZE;
  ...
  return <div className="performance-grid">
    <Panel title="表示期間" eyebrow="PERFORMANCE PERIOD" className="journal-wide period-filter-panel">
      <div className="period-tabs" role="group" aria-label="成績の表示期間">
      ...
      <p className="footnote" data-testid="period-summary">
        {label} · {counts.total} trades / {counts.closed} closed
        {counts.open > 0 ? `（保有中 ${counts.open}）` : ""}
      </p>
      {showRecentCompare && periodAvg !== null && allAvg !== null && (
        <p className="footnote">
          選択期間の平均損益は {money(periodAvg, true)}、全期間の平均損益は {money(allAvg, true)} です。
          {periodAvg > allAvg
            ? "直近の平均損益は全期間より高い記録です。"
            : periodAvg < allAvg
              ? "直近の平均損益は全期間より低い記録です。"
              : "直近と全期間の平均損益は同程度です。"}
          因果や将来の改善を保証するものではありません。
        </p>
      )}
    </Panel>
    ...
    {periodEmpty ? (
      ...
      <>
        ...
        <Panel title="損益カレンダー" eyebrow="MONTHLY P&L / JST" className="journal-wide">
          <div className="calendar-controls"><button type="button" disabled={month === "1000-01"} onClick={() => moveMonth(-1)}>前月</button><label>表示月<input type="month" min="1000-01" max="9999-12" value={month} onChange={e => { if (/^[1-9]\d{3}-(0[1-9]|1[0-2])$/.test(e.target.value)) { setMonth(e.target.value); setDay(null); } }} /></label><button type="button" disabled={month === "9999-12"} onClick={() => moveMonth(1)}>翌月</button></div>
          <p className="footnote">決済日の日本時間で合算（{label}）。緑＝利益、赤＝損失、0円＝引き分け、—＝取引なし。</p>
          <div className="pnl-calendar">{["月", "火", "水", "木", "金", "土", "日"].map(d => <span className="calendar-weekday" key={d}>{d}</span>)}{monthCells(month).map((date, i) => date ? <button type="button" key={date} aria-label={`${date} ${days[date] ? money(days[date].pnl, true) : "取引なし"}`} aria-pressed={day === date} className={days[date] ? tone(days[date].pnl) : "muted"} onClick={() => setDay(date)}><span>{Number(date.slice(-2))}</span><small>{days[date] ? money(days[date].pnl, true) : "—"}</small></button> : <span key={`empty-${i}`} />)}</div>
        ...
        <Panel title="AI判断との一致別" eyebrow="AI ALIGNMENT" className="journal-wide">
          <p className="footnote">登録時スナップショットと取引方向から計算します（{label}）。現在のAI状態ではありません。</p>
          <div className="performance-cards">{alignmentPerformance(filteredTrades).map(group => <article key={group.alignment}><h3>{alignmentLabels[group.alignment]}</h3><strong className={tone(group.totalPnl)}>{money(group.totalPnl, true)}</strong><p>{group.count}取引 · 勝率 {group.winRate === null ? "—" : `${group.winRate.toFixed(1)}%`}</p>{group.count === 0 ? <small className="muted">データなし</small> : group.insufficientData && <small className="neutral">参考値（データ不足）</small>}</article>)}</div>
        </Panel>
        <Panel title="AI判断別成績" eyebrow="SIGNAL REVIEW" className="journal-wide">
          <p className="footnote">登録時snapshotのsignalを使用します（{label}）。10件未満は「データ不足」。</p>
          <div className="performance-cards">{signalPerformance(filteredTrades).map(group => <article key={group.signal}><h3>{signalLabels[group.signal]}</h3><strong className={tone(group.totalPnl)}>{money(group.totalPnl, true)}</strong><p>{group.count}取引 · 勝率 {group.winRate === null ? "—" : `${group.winRate.toFixed(1)}%`}</p>{group.insufficientData && <small className="neutral">データ不足</small>}</article>)}</div>
        </Panel>
        <Panel title="Chart Evidence別" eyebrow="CHART USED" className="journal-wide">
          <div className="performance-cards">{chartEvidencePerformance(filteredTrades).map(group => <article key={group.key}><h3>{group.label}</h3><strong className={tone(group.totalPnl)}>{money(group.totalPnl, true)}</strong><p>{group.count}取引 · 勝率 {group.winRate === null ? "—" : `${group.winRate.toFixed(1)}%`}</p>{group.count === 0 ? <small className="muted">データなし</small> : group.insufficientData && <small className="neutral">参考値</small>}</article>)}</div>
        </Panel>
        <Panel title="確信度帯別（参考）" eyebrow="CONFIDENCE BANDS" className="journal-wide">
          <p className="footnote">サンプルが少ない場合は参考値です（{label}）。</p>
          <div className="performance-cards">{confidenceBandPerformance(filteredTrades).map(group => <article key={group.band}><h3>{group.band}%</h3><strong className={tone(group.totalPnl)}>{money(group.totalPnl, true)}</strong><p>{group.count}取引 · 勝率 {group.winRate === null ? "—" : `${group.winRate.toFixed(1)}%`}</p>{group.referenceOnly && <small className="neutral">参考値</small>}</article>)}</div>
        </Panel>
        <Panel title="通貨ペア別成績" eyebrow="PAIR REVIEW" className="journal-wide"><div className="performance-cards">{pairPerformance(filteredTrades).map(group => <article key={group.pair}><h3>{group.pair}</h3><strong className={tone(group.totalPnl)}>{money(group.totalPnl, true)}</strong><p>{group.count}取引 · 勝率 {group.winRate === null ? "—" : `${group.winRate.toFixed(1)}%`}</p></article>)}</div></Panel>
      </>
    )}
  </div>;
}
